...
function generatorInputFor({lead,input,mode,assetUrls=[],effects=[]}){return{name:lead?.name||String(input.name||"").trim(),segment:lead?.segment||input.segment,city:lead?.city||lead?.location||input.city,address:lead?.address||"",phone:lead?.phone||lead?.whatsapp||"",placeId:lead?.externalId||"",mapsLink:lead?.mapsLink||(mode==="google"?input.source:""),existingWebsite:lead?.site||"",instagram:lead?.instagram||"",rating:lead?.googleRating||"",reviews:lead?.googleReviews||"",description:mode==="lead"?leadDescription(lead):input.source,template:input.template||"landing",assetUrls,effects}}
...
export async function refineSiteProjectAction(input={}){
  ...
  const references=await listReferenceImages(referenceScope,{withData:true});
  const effectsChanged=JSON.stringify([...effects].sort())!==JSON.stringify([...(project.effects||[])].sort());
  if(!instruction&&!newReferences.length&&!effectsChanged)throw new Error("Descreva uma alteração, envie uma referência ou mude os efeitos.");
  let generatorInput={...(project.generatorInput||{}),effects};
  if(project.leadId){const lead=await getLead(project.leadId);if(!lead)throw new Error("O lead vinculado a este projeto não foi encontrado.");generatorInput=generatorInputFor({lead,input:{template:project.template},mode:"lead",assetUrls:await collectLeadAssetUrls(lead),effects})}
  const generated=await generateSiteFolder({...generatorInput,folderPath:project.folderPath,existingSiteData:project.siteData,instruction,referenceImages:references,skipAi:!instruction&&!newReferences.length});
  const updated=await updateSiteProject(project.id,{status:"ready",aiUsed:generated.aiUsed||project.aiUsed,warning:generated.warning,imageCount:generated.imageCount,siteData:generated.siteData,generatorInput,instructions:instruction?[...(project.instructions||[]),instruction]:(project.instructions||[]),version:Number(project.version||1)+1,effects,referenceScope,referenceImages:references});
