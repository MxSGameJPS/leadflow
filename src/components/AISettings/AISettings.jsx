...
export default function AISettings({ initialProviders = [], initialError = "" }) {
  ...
  async function save() {
    ...
    try {
      ...
      setMessage("Provedor salvo localmente. A chave foi armazenada de forma criptografada.");
    } catch (error) {
  ...
  async function test() {
    ...
    try {
      ...
      setMessage(`Conectado em ${result.elapsedMs} ms · HTTP ${result.status} · Resposta: ${result.text}`);
    } catch (error) {
      ...
      <div className={s.layout}>
        ...
        <main className={s.panel}>
          <div className={s.grid2}>
            <label>Nome do provedor<input value={form.name} onChange={event => set("name", event.target.value)} placeholder="Ex.: OpenRouter principal" /></label>
          ...
          <label>URL base<input value={form.baseUrl} onChange={event => set("baseUrl", event.target.value)} placeholder="https://api.exemplo.com/v1" /></label>
          ...
          <div className={s.grid2}>
            ...
            <label>Modelo<input list="ai-models" value={form.model} onChange={event => set("model", event.target.value)} placeholder="Nome exato do modelo" /><datalist id="ai-models">{models.map(model => <option key={model} value={model} />)}</datalist></label>
          </div>
