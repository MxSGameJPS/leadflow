...
export default function ProjectsList({ projects = [] }) {
  return <main className={s.page}>
    <header className={s.header}>
      <div><h1>Meus projetos</h1><p>Rascunhos e prévias vinculados aos leads do CRM.</p></div>
    ...
    {projects.length === 0 ? <section className={s.empty}><span>◇</span><h2>Nenhum projeto criado</h2><p>Abra um lead no CRM ou comece pelo criador de sites.</p><a href="/criar-site">Criar primeiro projeto</a></section>
      : <section className={s.grid}>{projects.map(project => <article className={s.card} key={project.id}>
        ...
        <dl><div><dt>Modelo</dt><dd>{TEMPLATE[project.template] || project.template}</dd></div><div><dt>Origem</dt><dd>{project.mode === "lead" ? "Lead do CRM" : project.mode === "google" ? "Link do Google" : "Descrição manual"}</dd></div><div><dt>Atualizado</dt><dd>{new Date(project.updatedAt).toLocaleString("pt-BR")}</dd></div></dl>
        <div className={s.actions}>{project.leadId && <a href={`/crm/${project.leadId}`}>Abrir lead</a>}<a href={`/criar-site${project.leadId ? `?lead=${project.leadId}` : ""}`}>Continuar projeto</a></div>
      </article>)}</section>}
  </main>;
}
