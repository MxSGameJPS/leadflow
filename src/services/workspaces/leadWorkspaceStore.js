...
const DEFAULT_WORKSPACE = Object.freeze({
  ...
  sale: { paymentTerms: "", meetingNotes: "", outcome: "open", projectValue: 0, paymentMethod: "Pix", installments: 1, paidInstallments: 0, amountPaid: 0, firstDueDate: "", paymentStatus: "pending" },
  consulting: {
    stage: "novo", status: "pending", websiteUrl: "", instagramUrl: "", instagramNotes: "", auditSnapshot: "", overallScore: 0,
    executiveSummary: "", visualSummary: "", report: "", whatsappMessage: "", priceCents: 5000, paymentStatus: "pending",
    soldAt: "", deliveredAt: "", lastAnalyzedAt: "", lastContactAt: "", diagnosisProviderName: "", diagnosisModel: "",
...
function normalizeWorkspace(input = {}) {
  ...
  try { commercialTrack = validateCommercialTrack(input.commercialTrack || "auto"); } catch { commercialTrack = "auto"; }
  const status = CONSULTING_STATUSES.has(consulting.status) ? consulting.status : DEFAULT_WORKSPACE.consulting.status;
  ...
  return {
    ...
    sale: {
      ...
      paymentMethod: cleanText(sale.paymentMethod || DEFAULT_WORKSPACE.sale.paymentMethod, 80), installments, paidInstallments, amountPaid,
      firstDueDate: cleanDate(sale.firstDueDate), paymentStatus: ["pending", "partial", "paid", "overdue"].includes(sale.paymentStatus) ? sale.paymentStatus : DEFAULT_WORKSPACE.sale.paymentStatus,
    },
    consulting: {
      stage: normalizeConsultingStage(consulting.stage), status, websiteUrl: cleanUrl(consulting.websiteUrl), instagramUrl: cleanUrl(consulting.instagramUrl),
      instagramNotes: cleanText(consulting.instagramNotes, 10_000), auditSnapshot: cleanText(consulting.auditSnapshot, 80_000),
      overallScore: cleanInteger(consulting.overallScore, 0, 0, 100), executiveSummary: cleanText(consulting.executiveSummary, 5000),
      visualSummary: cleanText(consulting.visualSummary, 6000), report: cleanText(consulting.report, 50_000), whatsappMessage: cleanText(consulting.whatsappMessage, 5000),
      priceCents: cleanInteger(consulting.priceCents, 5000, 0, 10_000_000), paymentStatus: ["pending", "paid", "refunded"].includes(consulting.paymentStatus) ? consulting.paymentStatus : "pending",
      soldAt: cleanTimestamp(consulting.soldAt), deliveredAt: cleanTimestamp(consulting.deliveredAt), lastAnalyzedAt: cleanTimestamp(consulting.lastAnalyzedAt), lastContactAt: cleanTimestamp(consulting.lastContactAt),
      diagnosisProviderName: cleanText(consulting.diagnosisProviderName || consulting.providerName, 180), diagnosisModel: cleanText(consulting.diagnosisModel || consulting.model, 180),
      reportProviderName: cleanText(consulting.reportProviderName, 180), reportModel: cleanText(consulting.reportModel, 180),
      diagnosisAiUsed: cleanBoolean(consulting.diagnosisAiUsed), reportAiUsed: cleanBoolean(consulting.reportAiUsed), warning: cleanText(consulting.warning, 2500),
    },
...
export async function saveLeadWorkspace(leadId, patch = {}) {
  ...
  const merged = normalizeWorkspace({ ...current, ...patch, appointment: { ...current.appointment, ...(patch.appointment || {}) }, sale: { ...current.sale, ...(patch.sale || {}) }, consulting: { ...current.consulting, ...(patch.consulting || {}) }, updatedAt: new Date().toISOString() });
